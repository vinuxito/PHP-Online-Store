...
(function(window, $) {
  ...
  class RoyalConciergeAgendaEngine extends window.QuantixBookingUI {}
  ...
  class QuantumComparisonStudio {
    ...
    renderFacts() {
      ...
      products.forEach(p=>{const cell=$('<th scope="col"></th>');if(p.cover)$('<img class="qx-facts-thumb" alt="">').attr('src',p.cover).appendTo(cell);cell.append($('<span></span>').text(p.name));table.find('thead tr').append(cell);});
      const rows=[['Precio',p=>Number(p.priceWithTax)>0?'$ '+store.formatMoney(p.priceWithTax)+' MXN':'Consultar precio'],['Categoría',p=>p.category],['Referencia',p=>p.sku||p.code],['Descripción',p=>{const text=window.QuantixDesignContract.plain(p.storeDesc||p.notes||'');return text.length>220?text.slice(0,217)+'…':text;}],['Disponibilidad',p=>store.tenant.showStock!==false&&p.inInventory?String(p.stock)+' unidades':'Consultar disponibilidad']];
...
${shareUrl}`;
  ...
  class QuantixStorefront {
    ...
    openAgendaModal(context = {}) {
      ...
      this.royalAgenda.openAgendaModal(context);
    }
    ...
    requestPropertyContact(product, tour = false) {
      if (tour && this.tenant && this.tenant.featureMatrix && this.tenant.featureMatrix.royal_agenda && this.tenant.featureMatrix.royal_agenda.enabled) {
        ...
        this.openAgendaModal({ productId: product.id, title: product.name, image: product.cover || (product.photos && product.photos[0] && (product.photos[0].url || product.photos[0].thumb)) || '' });
        return;
      }
    ...
    createCardElement(p, globalIdx = 0) {
      ...
      if (window.QuantixDesignContract.capabilities(self.tenant).shop && amount > 0) actions.append($('<button type="button" class="qx-design-add"></button>').attr('aria-label', (p.hasDecant?'Elegir formato: ':'Agregar al carrito: ') + p.name).text(p.hasDecant?'Elegir formato':'Agregar').on('click', function() { if (p.hasDecant) self.openProductModal(p); else self.addToCart(p, 1, media.find('img')); }));
